...
tcpServer.on("connection", function (socket) {
  ...
  const onMessage = (message) => {
    ...
    if (streamUpFrame.useEncrypt) {
      ...
      const endToEndKey = streamUpFrame.key;
      ...
      socket.resume();
      console.info(
        "ws streamUpFrame found, start pipe (use encrypt: true)",
        socketInfo
      );
      removeOnMessage();
      // 处理加密解密流错误
      encryptStream.on("error", (err) => {
        console.error("Encrypt stream error:", err.message, socketInfo);
        wsStream.end();
        socket.end();
        ws.close();
        encryptStream.end();
      ...
      decryptStream.on("error", (err) => {
        console.error("Decrypt stream error:", err.message, socketInfo);
        wsStream.end();
        socket.end();
        ws.close();
        decryptStream.end();
  ...
  socket.on("error", (err) => {
    ...
    clearInterval(pingInterval);
    console.info(
      "Close connection due to socket error",
      err.message,
      socketInfo
    );
  });
